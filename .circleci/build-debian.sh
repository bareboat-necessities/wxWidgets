...
docker exec --privileged -ti $DOCKER_CONTAINER_ID /bin/bash -xec \
    "update-alternatives --set fakeroot /usr/bin/fakeroot-tcp; cd ci-source/work; echo DISTRIBUTION=bookworm > ~/.pbuilderrc; curl -o /usr/share/pbuilder/pbuilderrc 'http://www.mirbsd.org/cvs.cgi/~checkout~/contrib/hosted/tg/deb/pbuilderrc?rev=1.101;content-type=text%2Fplain' ; dpkg-buildpackage -uc -us -j8; mkdir dist; mv ../*.deb dist; chmod -R a+rw dist"
